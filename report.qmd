...
bball_sum_stats <- mens_bball_sum_stats |>
  ...
  mutate(men_diff = avg_rev_men - avg_exp_men) |>
  mutate(women_diff = avg_rev_women - avg_exp_women) |>
  mutate(men_diff = as.factor(men_diff)) |>
  mutate(women_diff = as.factor(women_diff))
...
```{r}
#| label: differece-facet

bball_sum_stats |>
  ggplot(
   aes(x = men_diff, group = "Institution.Name")) +
    geom_bar() +
    labs(
      title = "Relationship Between Sex and Revenue:Expenses Ratio
      in Basketball in the ACC 2020-2021",
