...
mens_bball_sum_stats <- basketball |>
  ...
  pivot_longer(avg_rev_men:avg_exp_men,
               names_to = "mens_stat", values_to = "value1")
...
womens_bball_sum_stats <- basketball |>
  ...
  pivot_longer(avg_rev_women:avg_exp_women,
               names_to = "womens_stat", values_to = "value2")

bball_sum_stats <- mens_bball_sum_stats |>
  left_join(womens_bball_sum_stats) 

#bball_sum_stats <- bball_sum_stats |> 
  #mutate(sex = if_else(grepl("women", stat), "Women's", "Men's"))

#bball_sum_stats <- bball_sum_stats |> 
  #mutate(metric = if_else(grepl("rev", stat), "Revenue", "Expenses"))
...
bball_sum_stats |>
  ggplot(
    ...
    labs(
      ...
      x = "Above or Below Average Age (29.9 years)",
      y = "Relative Frequency of Survival", 
      fill = "Survived"
    )
